    ...
    public class PostAssessmentItemsController : Controller
    {
        ...
        public async Task<ActionResult> Create([Bind(Include = "Id,Name,IconURL,IsPostviewable,PostLessonId,PostLessonId,RecLog")] AssessmentItem assessmentItem)
        {
            if (ModelState.IsValid)
            {
                ...
                assessmentItem.Order = lesson.PostAssessments.Where(it => !it.RecLog.DeletedDate.HasValue).Count() + 1;
        ...
        public async Task<ActionResult> DeleteConfirmed(int id)
        {
            ...
            var lessonId = assessmentItem.PostLessonId;

            foreach (var item in assessmentItem.Assessments) item.RecLog.DeletedDate = now;

            var deleteOrder = assessmentItem.Order;
            var AssessmentList = db.AssessmentItems.Where(it => !it.RecLog.DeletedDate.HasValue && it.PostLessonId == lessonId && it.Order > deleteOrder);
